...
void Advect2D::updateGodunov(HaloArray2D *u) {
  timer->start("updateGodunov", u->l.prod(), 1, myCommWorld);
  ...
  delete f_x;
  delete f_y;
  timer->stop("updateGodunov", myCommWorld);
} //updateGodunov()

void Advect2D::updateLW(HaloArray2D *u) {
  timer->start("updateLW", u->l.prod(), 1, myCommWorld);
  ...
  delete uh;
  timer->stop("updateLW", myCommWorld);
} //updateLW()

void Advect2D::updateMacCormack(HaloArray2D *u) {
  timer->start("updateMacCormack", u->l.prod(), 1, myCommWorld);
  ...
  delete up;
  timer->stop("updateMacCormack", myCommWorld);
}
...
void Advect2D::updateBoundary(HaloArray2D *u, ProcGrid2D *g, int test) {
  assert(u->halo == 1); 
  timer->start("updateBoundary", u->l.prod(), 1, myCommWorld);
  ...
  timer->stop("updateBoundary", myCommWorld);
} //updateBoundary()
